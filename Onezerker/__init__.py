...
class Onezerker(bl2sdk.BL2MOD):
    ...
    def debugLogging(self, caller, function, params):
        if self.LOGGING == self.LoggingLevel.CALLS:
            bl2sdk.Log("[Onezerker] " + str(function).split(".")[-1])
        elif self.LOGGING == self.LoggingLevel.FULL:
            bl2sdk.Log("[Onezerker] " + str(caller))
            bl2sdk.Log("[Onezerker] " + str(function))
            bl2sdk.Log("[Onezerker] " + str(params))
    ...
    def Enable(self):
        ...
        def EndClimbLadder(caller: bl2sdk.UObject, function: bl2sdk.UFunction, params: bl2sdk.FStruct) -> bool:
            self.debugLogging(caller, function, params)
            
            # Don't do anything special if this isn't our pawn or when not gunzerking
            if caller.MyActionSkill == None:
                return True
        ...
        bl2sdk.RegisterHook("WillowGame.WillowPlayerPawn.EndClimbLadder", "Onezerker", EndClimbLadder)
    ...
    def Disable(self):
        ...
        bl2sdk.RemoveHook("WillowGame.WillowPlayerPawn.EndClimbLadder", "Onezerker")
