    ...
    def Enable(self) -> None:
        ...
        def OnUpdate() -> None:
            ...
            for vendor in unrealsdk.FindAll("WillowVendingMachine"):
                ...
                if vendor.ShopType != 1 and vendor.ShopType != 2:  # SType_Health
                    continue

                if vendor.InteractiveObjectDefinition is None:
                    continue

                cost: int
                if vendor.ShopType == 2:
                    vendor.InteractiveObjectDefinition.HUDIconDefSecondary = self.HealthIcon
                    cost = self.GetHealthCost(closetPawn, vendor)
                else:
                    vendor.InteractiveObjectDefinition.HUDIconDefSecondary = self.AmmoIcon
        ...
        AsyncUtil.RunEvery(0.25, OnUpdate, self.Name)
